...
from backend.future_integration import FutureIntegration
...
from backend.future_integration.router import FutureIntegration
...
class AIEngine:
    ...
    def __init__(self) -> None:
        ...
        self.future_integration = FutureIntegration(self)
    ...
    def understand_question(self, message: str) -> dict:
        ...
        if any(
            ...
            programming_text = message.lower()
    ...
    def generate(
        ...
        # =========================================================
        # FUTURE INTELLIGENCE ROUTER
        # =========================================================
        # Future questions are handled by the completed Future System.
        # Normal ISMAIL AI questions continue through the existing flow.
        if not hasattr(self, "_future_integration"):
            self._future_integration = FutureIntegration(self)
        future_result = self._future_integration.analyze(
            message,
            user_id=user_id,
            chat_id=chat_id,
        )
        if future_result is not None:
            if future_result.get("status") == "success":
                natural_answer = future_result.get("natural_answer")
                if natural_answer:
                    return natural_answer
                answer = future_result.get("answer")
                if answer:
                    return answer
                # If the Future System returned a structured result
                # without a natural answer, let the existing AI explain
                # the result rather than returning raw internal data.
                future_prompt = (
                    "Answer the user's future-prediction question using "
                    "the following Future Intelligence System result. "
                    "Do not invent facts. Clearly distinguish forecast, "
                    "probability, confidence and uncertainty. "
                    "Do not claim certainty.\n\n"
                    f"USER QUESTION:\n{message}\n\n"
                    f"FUTURE SYSTEM RESULT:\n{json.dumps(future_result, ensure_ascii=False, default=str)}"
                )
                return self.llm_router.generate(future_prompt)
            # A Future System failure must NOT break the existing AI.
            # Continue with the original ISMAIL flow.
            # =========================================================
        ...
        if self.provider in ("openai", "openrouter", "groq"):
            # ------------------------------------------------
            # Future Intelligence route
            # ------------------------------------------------
            # Specialized existing routes above remain untouched.
            # FutureIntegration uses the existing ISMAIL AI
            # as the final reasoning/language layer.
            try:
                future_result = self.future_integration.analyze(
                    message=message,
                ...
                if isinstance(future_result, dict):
                    if future_result.get("status") == "success":
                        ...
                        if isinstance(answer, dict):
                            ...
                            if ai_reasoning:
                                return str(ai_reasoning)
                            if summary:
                                return str(summary)
                        ai_reasoning = future_result.get(
                            "ai_reasoning"
                        )
                        if ai_reasoning:
                            return str(ai_reasoning)
            except Exception:
                # Future intelligence must never break
                # the existing ISMAIL AI.
                pass
