...
def _verify_evidence_agreement(
    ...
    for index, item in enumerate(evidence):
        ...
        #
        # This does NOT mark the evidence as corroborated.
        # It only preserves strong primary-source evidence so that
        # broad live queries do not become empty.
        if (
            item.reliability_score >= 0.90
            and len(current_words) >= 3
            and current_domain
        ):
            item.verification_status = "high_confidence_source"
            continue
...
class WebResearch:
    ...
    def _extract_google_news_date(self, item_xml: str) -> Optional[datetime]:
    ...
    def search(self, question: str, max_sources: int = 5):
        ...
        news_query = bool(re.search(
            ...
            r"|খবর|সর্বশেষ|আজকের|আজ|এখন|বর্তমানে|ব্রেকিং|সংবাদ|"
            r"সাম্প্রতিকতম|সাম্প্রতিক|ঘটনা|ঘটনাগুলো|ঘটনাসমূহ|"
            r"কী ঘটছে|কি ঘটছে|কী হচ্ছে|কি হচ্ছে|"
            r"গুরুত্বপূর্ণ ঘটনা|বর্তমান ঘটনা|সাম্প্রতিক ঘটনা|আন্তর্জাতিক ঘটনা",
            question,
            re.IGNORECASE,
        ))
        ...
        global_news = bool(
            re.search(
                r"world|global|international|বিশ্ব|আন্তর্জাতিক|বিশ্বজুড়ে|বিশ্বব্যাপী",
                question,
                re.IGNORECASE,
            )
            and news_query
            and not bangladesh_news
        )
        ...
        if news_query:
            try:
                ...
                for item_xml in item_pattern.findall(google_news_xml):
                    ...
                    if (bangladesh_news or global_news) and published_at is not None:
                        now_utc = datetime.now(timezone.utc)
                    ...
                    # ------------------------------------------------------------
                    # Strict relevance filter for Bangladesh / Global live news
                    # ------------------------------------------------------------

                    title_lower = title.lower()

                    # Broad current-event queries should prefer
                    # genuinely current developments, not anniversary/
                    # remembrance/history coverage.
                    # Reject generic pages and non-event headlines
                    # for broad current-event searches.
                    if broad_current_events:
                        ...
                        historical_event_patterns = [
                            ...
                            r"\bseptember\s+11\b",
                            r"\bsept(?:ember)?\s+11\b",
                            r"\b9/11\s+anniversary\b",
                            r"\b25th\s+anniversary\b",
                            r"9/11-এর\s+২৫তম\s+বার্ষিকী",
                            r"৯/১১",
                            r"বার্ষিকী",
                            r"anniversary",
                            r"remembering",
                        ...
                        if any(
                            ...
                            for pattern in historical_event_patterns
                        ):
                            continue
                    ...
                    if any(re.search(pattern, title_lower, re.IGNORECASE)
                        for pattern in spam_patterns):
                        continue

                    # Never accept obvious social-media posts as primary live-news evidence.
                    if re.search(
                        r"facebook\.com|youtube\.com|instagram\.com|tiktok\.com",
                        title_lower,
                        re.IGNORECASE,
                    ):
                        continue

                    # Bangladesh query must contain Bangladesh-related signals.
                    if bangladesh_news:
                        bangladesh_signals = [
                            "bangladesh",
                            "বাংলাদেশ",
                    ...
                    # Global query must not accidentally accept Bangladesh-only content.
                    if global_news:
                        if re.search(
                            r"\b(bangladesh|dhaka|chittagong|sylhet)\b|"
                            r"বাংলাদেশ|ঢাকা|চট্টগ্রাম|সিলেট",
                            title_lower,
                            re.IGNORECASE,
                        ):
                            continue
        ...
        try:
                ...
                for title, url, snippet in parsed:
                    ...
                    if broad_current_events:
                        generic_page_patterns = [
                            r"\bwar today\b",
                            r"\blive war news\b",
                            r"\bevery active conflict\b",

                            r"\blive:\s*abc news\b",
                            r"\babc news live\b",
                            r"\bnews live\b",
                            r"\b24/7 news\b",
                            r"\bnews 24/7\b",
                            r"\blive breaking news\b",
                            r"\bbreaking news and top stories\b",
                        ...
                        if broad_current_events:
                            ...
                            if any(
                                re.search(pattern, url or "", re.IGNORECASE)
            ...
            print("GOOGLE NEWS ERROR:", repr(e))
        ...
        try:
            ...
            for title, url, snippet in bing_results:
                ...
                # ------------------------------------------------------------
                # Broad current-events query: reject obvious Bing junk
                # ------------------------------------------------------------
                if broad_current_events:
                    junk_patterns = [
                        # Search engines / portals / generic pages
                        r"yandex",
                        r"百度知道",
                        r"baidu",
                        r"microsoft community",
                        ...
                        # Games / cheats / unrelated knowledge
                        r"age of empires",
                        r"秘籍",
                        r"cheat",
                        r"cheats",
                        ...
                        # Math / reference pages
                        r"π等于多少",
                        r"\bpi equals\b",
                        r"\bwhat is pi\b",

                        # Generic news portals
                        r"ms now",
                        r"msnbc",
                        r"newsbreak",
                        r"personalized news",
                    ...
                    combined_text = (
                        f"{title_lower} {snippet_lower} {url_lower}"
                    )
                    ...
                    if any(
                        ...
                        print("BING FILTERED JUNK:", title)
                        continue

                    # Social/video results are not suitable for broad event verification
                    if re.search(
                        r"youtube\.com|youtu\.be|facebook\.com|"
                        r"instagram\.com|tiktok\.com",
                        url_lower,
                        re.IGNORECASE,
                    ):
                        print("BING FILTERED SOCIAL:", title)
                        continue
        ...
        for part in parts:
            ...
            needs_search = bool(
                re.search(
                    ...
                    r"score|scores"
                    r"|event|events|incident|incidents|"
                    r"happening|happenings"
                    r")\b"
                    r"|খবর|সর্বশেষ|আজকের|আজ|ব্রেকিং|সংবাদ|"
                    r"সাম্প্রতিকতম|সাম্প্রতিক|স্ট্যাটাস|"
                    r"ঘটনা|ঘটনাগুলো|ঘটনাসমূহ|কী ঘটছে|কি ঘটছে|"
                    r"কী হচ্ছে|কি হচ্ছে|গুরুত্বপূর্ণ ঘটনা|বর্তমান ঘটনা|"
                    r"সাম্প্রতিক ঘটনা|আন্তর্জাতিক ঘটনা|"
