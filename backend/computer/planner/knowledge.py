...
def search_computer_action_plan(query):
    ...
    # Bengali fallback patterns were removed because the previous
    # block contained corrupted console-encoded phrases and caused
    # broad false-positive computer matches.
    ...
    # Controlled Bengali computer commands.
    # Exact normalized phrases only; no broad substring matching.
    controlled_bengali_actions = {
        "open_application": [
            "chrome ???? ???",
            "chrome ????",
            "????? ???? ???",
    ...
    for action_name, phrases in controlled_bengali_actions.items():
